...
export const testEmbeddings = async (req, res) => {
  try {
    ...
    res.json({
      status: 'Embedding system working!',
      totalUsers,
      usersWithEmbeddings,
      embeddingPercentage: totalUsers > 0 ? ((usersWithEmbeddings / totalUsers) * 100).toFixed(1) + '%' : '0%'
    });
  } catch (error) {
    res.json({ error: error.message });
  }
