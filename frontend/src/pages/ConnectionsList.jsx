...
const ConnectionsList = () => {
  ...
  const fetchConnections = async () => {
    try {
      ...
      setConnections(response.data.connections?.list || []);
    } catch (error) {
  ...
  if (loading) {
    return (
      <div className="fixed inset-0 bg-gradient-to-br from-[#E8FFD7] via-[#93DA97] to-[#5E936C] p-6 flex items-center justify-center">
        <div className="text-center">
          <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-green-800 mx-auto mb-4"></div>
          <p className="text-green-800 font-medium">Loading connections...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="connection-list fixed inset-0 bg-gradient-to-br from-[#E8FFD7] via-[#93DA97] to-[#5E936C] overflow-auto">
      <div className="w-full h-full">
        <div className="flex items-center mb-6 p-6 pb-0">
          ...
          <h1 className="text-3xl font-bold text-green-800">Connections ({connections.length})</h1>
        </div>

        {connections.length === 0 ? (
          <div className="text-center py-16">
            <div className="text-6xl mb-4">🤝</div>
            <h3 className="text-2xl font-semibold text-green-800 mb-2">No connections yet</h3>
            <p className="text-green-600">Start connecting with people to see them here</p>
          </div>
        ) : (
              ...
              <table className="w-full min-w-full">
                ...
                <tbody>
                  {connections.map((connection, index) => (
                    <tr key={connection._id} className={`border-b border-green-200 hover:bg-green-50 ${index % 2 === 0 ? 'bg-white/50' : 'bg-green-50/30'}`}>
                      ...
                      <td className="px-6 py-4 text-green-700">
                        {connection.age && connection.location ? `${connection.age} • ${connection.location}` : '-'}
                      </td>
                      <td className="px-6 py-4 text-green-700">
                      ...
                      <td className="px-6 py-4 text-green-700 max-w-xs">
                        {connection.bio ? (
                          <span className="italic text-sm">"{connection.bio.length > 50 ? connection.bio.substring(0, 50) + '...' : connection.bio}"</span>
                        ) : '-'}
                      </td>
                      <td className="px-6 py-4">
                        {connection.hobbies && connection.hobbies.length > 0 ? (
                          <div className="flex flex-wrap gap-1">
                            {connection.hobbies.slice(0, 2).map((hobby, idx) => (
                              <span key={idx} className="bg-green-200 text-green-800 px-2 py-1 rounded-full text-xs">
                                {hobby}
                              </span>
                            ))}
                            {connection.hobbies.length > 2 && (
                              <span className="text-green-600 text-xs">+{connection.hobbies.length - 2}</span>
                            )}
                          </div>
                        ) : '-'}
                      </td>
